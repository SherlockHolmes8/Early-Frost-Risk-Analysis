...
#CMD uvicorn api_server:app --host 0.0.0.0 --port 8050 & streamlit run dashboard.py --server.port $PORT --server.address 0.0.0.0 --server.enableCORS=false --server.enableXsrfProtection=false #Render Web Service İçin